...
public class DbHelperBudget {

    private final static String TAG = "DbHelperBudget";
    private final static String EQUAL_QUESTION_MARK = "=?";
    private static final String SYMBOL_OR = " OR ";

    public static Uri addBudget(Context context, Budget budget) {
        ContentValues values = new ContentValues();
        values.put(DbProvider.COL_BUDGET_CATEGORY, budget.getCategory());
        values.put(DbProvider.COL_BUDGET_TYPE, budget.getType());
        values.put(DbProvider.COL_BUDGET_NOTE, budget.getNote());
        values.put(DbProvider.COL_BUDGET_DATE, budget.getDate());
        values.put(DbProvider.COL_BUDGET_AMOUNT, budget.getAmount());
        Uri uri = context.getContentResolver().insert(DbProvider.CONTENT_URI_BUDGET, values);
        Log.d(TAG, "New transaction added : " + uri.toString());
        return uri;
    }

    public static int updateBudget(Context context, Budget budget) {

        String whereClause = DbProvider.COL_BUDGET_ID + EQUAL_QUESTION_MARK;
        String[] whereArgs = {String.valueOf(budget.getId())};

        ContentValues values = new ContentValues();
        values.put(DbProvider.COL_BUDGET_ID, budget.getId());
        values.put(DbProvider.COL_BUDGET_CATEGORY, budget.getCategory());
        values.put(DbProvider.COL_BUDGET_TYPE, budget.getType());
        values.put(DbProvider.COL_BUDGET_NOTE, budget.getNote());
        values.put(DbProvider.COL_BUDGET_DATE, budget.getDate());
        ...
        int row = context.getContentResolver().update(DbProvider.CONTENT_URI_BUDGET, values, whereClause, whereArgs);
        Log.d(TAG, "Updated row : " + row);
        return row;
    }

    public static int deleteBudget(Context context, String budgetId) {
        String whereClause = DbProvider.COL_BUDGET_ID + EQUAL_QUESTION_MARK;
        String[] whereArgs = {budgetId};
        int row = context.getContentResolver().delete(DbProvider.CONTENT_URI_BUDGET, whereClause, whereArgs);
        Log.d(TAG, "Deleted row : " + row);
        return row;
    }

    public static Budget getBudget(Context context, String budgetId) {

        String whereClause = DbProvider.COL_BUDGET_ID + EQUAL_QUESTION_MARK;
        String[] whereArgs = {budgetId};

        Cursor cursor = context.getContentResolver().query(DbProvider.CONTENT_URI_BUDGET,
        ...
        if (cursor.getCount() > 0 && cursor.moveToFirst()) {
            ...
            budget.setId(getIntVal(cursor, DbProvider.COL_BUDGET_ID));
            budget.setCategory(getStrVal(cursor, DbProvider.COL_BUDGET_CATEGORY));
            budget.setNote(getStrVal(cursor, DbProvider.COL_BUDGET_NOTE));
            budget.setDate(getStrVal(cursor, DbProvider.COL_BUDGET_DATE));
            budget.setAmount(getIntVal(cursor, DbProvider.COL_BUDGET_AMOUNT));
            budget.setType(getIntVal(cursor, DbProvider.COL_BUDGET_TYPE));
    ...
    public static List<Budget> getBudgetList(Context context) {
        ...
        if (cursor.getCount() > 0) {
            List budgetList = new ArrayList<Budget>();
            cursor.moveToFirst();
